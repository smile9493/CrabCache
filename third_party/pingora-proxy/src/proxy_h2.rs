    ...
    C: custom::Connector,
{
        ...
        SV::CTX: Send + Sync,
    {
        ...
        // retry, send buffer if it exists (or overflowed — body may be supplied in request_body_filter)
        // Also send when downstream body was fully consumed in request_filter
        // (is_body_done true but buffer truncated flag not set — body read manually,
        //  retry buffer never populated).
            ...
            || self.inner.defer_upstream_request_body(session, ctx)
        {
            ...
            .await?;
        } else if downstream_state.is_done() {
            self.send_body_to2(
                session,
                None,
                downstream_state.is_done(),
