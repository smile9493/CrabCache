...
pub trait Peer: Display + Clone {
    ...
    /// Timeout waiting for H2 PONG after sending a PING frame.
    fn h2_ping_timeout(&self) -> Option<Duration> {
        self.get_peer_options().and_then(|o| o.h2_ping_timeout)
    }
...
pub struct PeerOptions {
    ...
    /// Timeout waiting for H2 PONG after sending a PING frame.
    /// If `None`, defaults to 5 seconds (the legacy pingora constant).
...
impl Display for PeerOptions {
    fn fmt(&self, f: &mut Formatter<'_>) -> FmtResult {
        ...
        if let Some(h2_ping_timeout) = self.h2_ping_timeout {
            write!(f, "h2_ping_timeout: {:?},", h2_ping_timeout)?;
        }
        Ok(())
    }
}
