...
impl StreamingChoice {
    fn to_message(&self) -> Value {
        ...
        msg.insert("role".into(), Value::String(if self.role.is_empty() { "assistant".to_string() } else { self.role.clone() }));
        msg.insert("content".into(), Value::String(self.content.clone()));
        if self.has_reasoning_content {
            msg.insert("reasoning_content".into(), Value::String(self.reasoning_content.clone()));
        }
...
impl StreamAccumulator {
    ...
    pub fn ingest_chunk(&mut self, chunk: &Value) {
        ...
        for raw_choice in choices {
            ...
            let index = raw_choice.get("index").and_then(|i| i.as_u64()).unwrap_or(0) as usize;
            ...
            if let Some(tool_calls) = delta.get("tool_calls").and_then(|tc| tc.as_array()) {
                for raw_delta in tool_calls {
                    if !raw_delta.is_object() { continue; }
                    let tc_index = raw_delta.get("index").and_then(|i| i.as_u64()).unwrap_or(choice.tool_calls.len() as u64) as usize;
                    ...
                    if let Some(tc) = tool_call.as_object_mut() {
                        if let Some(id) = raw_delta.get("id") { tc.insert("id".into(), id.clone()); }
                        if let Some(t) = raw_delta.get("type") { tc.insert("type".into(), t.clone()); }
                        if let Some(func_delta) = raw_delta.get("function") {
                            ...
                            if let Some(func_obj) = func.as_object_mut() {
                                if let Some(name) = func_delta.get("name").and_then(|n| n.as_str()) {
                                    let existing = func_obj.get("name").and_then(|n| n.as_str()).unwrap_or("").to_string();
                                    func_obj.insert("name".into(), Value::String(if existing.is_empty() { name.to_string() } else { format!("{}{}", existing, name) }));
                                }
                                if let Some(args) = func_delta.get("arguments") {
                                    let existing = func_obj.get("arguments").and_then(|a| a.as_str()).unwrap_or("").to_string();
                                    let new_args = args.as_str().unwrap_or("");
                                    func_obj.insert("arguments".into(), Value::String(format!("{}{}", existing, new_args)));
                                }
                            }
                        }
                    }
    ...
    pub fn store_ready_reasoning(
        ...
        for (index, msg) in messages.iter().enumerate() {
            ...
            let stage = if choice.map_or(false, |c| c.finish_reason.is_some()) { "final" }
                else if choice.map_or(false, |c| has_identified_tool_calls(c)) { "tool_call" }
                else { continue };

            let stage_rank = |s: &str| -> u8 { match s { "tool_call" => 1, "final" => 2, _ => 0 } };
            let storage_key = (index, scope.to_string());
            if let Some(previous) = self.stored_choices.get(&storage_key) {
                if stage_rank(previous) >= stage_rank(stage) { continue; }
            }
            let count = store.store_assistant_message(msg, scope, cache_namespace, prior_messages);
            if count > 0 { self.stored_choices.insert(storage_key, stage.to_string()); }
            stored += count;
        }
        stored
    }
    ...
    pub fn messages(&self) -> Vec<Value> {
        ...
        entries.iter().map(|(_, choice)| choice.to_message()).collect()
    }
}

fn has_identified_tool_calls(choice: &StreamingChoice) -> bool {
    if !choice.has_reasoning_content || choice.tool_calls.is_empty() { return false; }
...
impl CursorReasoningDisplayAdapter {
    pub fn new(collapsible: bool) -> Self {
        Self {
            ...
            block_start: if collapsible { COLLAPSIBLE_THINKING_BLOCK_START.to_string() } else { THINKING_BLOCK_START.to_string() },
            block_end: if collapsible { COLLAPSIBLE_THINKING_BLOCK_END.to_string() } else { THINKING_BLOCK_END.to_string() },
        }
    }
    ...
    pub fn rewrite_chunk(&mut self, chunk: &mut Value) {
        ...
        for raw_choice in choices.iter_mut() {
            ...
            let (reasoning_content, existing_content, has_tool_calls, has_finish) = {
                ...
                let rc = delta.get("reasoning_content").and_then(|r| r.as_str()).unwrap_or("").to_string();
                let ec = delta.get("content").and_then(|c| c.as_str()).unwrap_or("").to_string();
            ...
            let should_close = self.open_choices.contains_key(&index) && (has_content || has_tool_calls || has_finish);
            if should_close {
                mirrored_parts.push(self.block_end.clone());
                self.open_choices.remove(&index);
            }

            if mirrored_parts.is_empty() { continue; }
            if has_content { mirrored_parts.push(existing_content); }
    ...
    pub fn flush_chunk(&mut self, model: &str) -> Option<Value> {
        if self.open_choices.is_empty() { return None; }

        let mut sorted_keys: Vec<&usize> = self.open_choices.keys().collect();
        sorted_keys.sort();

        let choices: Vec<Value> = sorted_keys.iter().map(|&&index| {
            serde_json::json!({
                "index": index,
    ...
    fn remember_chunk_metadata(&mut self, chunk: &Value) {
        for key in &["id", "object", "created"] {
            if let Some(val) = chunk.get(key) {
                self.last_chunk_metadata.insert(key.to_string(), val.clone());
            }
        }
    }
}
...
pub fn fold_reasoning_into_content(response_payload: &mut Value, collapsible: bool) {
    let block_start = if collapsible { COLLAPSIBLE_THINKING_BLOCK_START } else { THINKING_BLOCK_START };
    let block_end = if collapsible { COLLAPSIBLE_THINKING_BLOCK_END } else { THINKING_BLOCK_END };
    ...
    for choice in choices.iter_mut() {
        ...
        let (reasoning, content) = {
            ...
            let r = message.get("reasoning_content").and_then(|r| r.as_str()).unwrap_or("").to_string();
            let c = message.get("content").and_then(|c| c.as_str()).unwrap_or("").to_string();
            (r, c)
        };
        if reasoning.is_empty() { continue; }
        if let Some(message) = choice.get_mut("message") {
            if let Some(obj) = message.as_object_mut() {
                obj.insert("content".into(), Value::String(format!("{}{}{}{}", block_start, reasoning, block_end, content)));
            }
        }
    }
}
...
mod tests {
    ...
    fn test_stream_accumulator_ingest() {
        ...
        assert_eq!(msgs[0].get("content").and_then(|c| c.as_str()), Some("Hello"));
    }
    ...
    fn test_fold_reasoning_into_content() {
        ...
        let content = payload.get("choices").unwrap().get(0).unwrap().get("message").unwrap().get("content").unwrap().as_str().unwrap();
