...
pub fn ReasoningPage() -> impl IntoView {
    ...
    view! {
        {move || match config.get() {
            None => view! { <div class="config-card glass-card"><crate::components::skeleton::SkeletonTable rows=5 cols=2 /></div> }.into_any(),
                ...
                view! {
                    <div class="config-card glass-card">
                        <div class="config-card-head">
                            <h3 class="config-card-title">{t.reasoning_thinking_mode()}</h3>
                        </div>
                        <div class="config-card-body space-y-4">
                            <select
                                class="config-input w-full"
                                prop:value=move || thinking_mode.get()
                                on:change=move |ev| thinking_mode.set(event_target_value(&ev))
                            >
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    on:change=move |ev| reasoning_effort.set(event_target_value(&ev))
                                >
                            ...
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input
                                    ...
                                    on:change=move |ev| reasoning_recovery.set(event_target_checked(&ev))
                                />
                                ...
                                view! {
                                    <div class="ml-6">
                                        ...
                                        <input
                                            ...
                                            on:input=move |ev| sqlite_cache_path.set(event_target_value(&ev))
