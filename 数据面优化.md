...
> **运维 / 事故**：内测 Trace 与 MiMo 429 等见 [docs/OPS_RUNBOOK.md](docs/OPS_RUNBOOK.md)（**非**仅靠数据面 P3 可解决）。  
...
> [!NOTE]
> 本文聚焦数据面（proxy 请求热路径），不覆盖控制面（Management API、Admin Dashboard）和部署运维层面。控制面、上游 Key 池、Trace 截断等见 [OPS_RUNBOOK.md](docs/OPS_RUNBOOK.md)。

---

## 运行时分析对照与遗漏项

对照 **2026-05 内测 Trace**（约 12k 行、综合命中率 ~25%、MiMo 高峰 429）。**P0–P2 数据面已落地项**（Bytes、early MiMo cache、prefix 索引、prewarm 等）已证明 **缓存 HIT 路径与阶段指标正常**；报告中的主痛点需 **多轨并行**，勿误判为「数据面未完成」。

### 与数据面矩阵的关系

| 报告问题 | 继续做完本文矩阵能否明显改善 | 备注 |
|----------|------------------------------|------|
| MiMo 上游 429 / Key 池耗尽 | **否** | 运维：加 Profile 上游 Key；矩阵无 per-key in-flight |
| 命中率 25% vs 目标 98% | **部分** | 内测大 body、低重复；L2 语义、近似键有帮助，达不到 98% |
| MISS P99 高 | **有限** | 上游推理主导；数据面仅省 parse/哈希毫秒级 |
| Coalescing Follower 雪崩 | **未覆盖** | 设计不重试上游；可选近似键减轻 Leader 压力 |
| Trace 截断、Redis 启动 | **否** | 见 OPS_RUNBOOK、PERSISTENCE、OBSERVABILITY |

**命中率 98%**：适用于高重复流量；Cursor 长线程内测请用 `hit_rate_5m` 等，见 [DATA_PLANE.md](docs/DATA_PLANE.md#运行时分析对照2026-05-内测)。

### 建议并入矩阵的「非数据面」轨（单独排期）

| 项 | 轨道 | 优先级建议 |
|----|------|------------|
| Profile 多上游 Key + 冷却可观测 | 运维 / Management | **P0** |
| 每上游 Key 最大 in-flight | 网关产品 | P1 |
| 429/503 `limit_source` 结构化 | 控制面 | P1 |
| 上游连续 429 熔断 | 网关 | P2 |
| Coalescing Leader 失败策略（Retry-After） | 产品 | P2 |
| Trace / Admin 读取与轮转 | Admin | P1 |

### 数据面侧下一批 ROI（内测场景）

1. **启用 L2 语义缓存**（配置项，非 proxy 单行改动）  
2. **MiMo 近似缓存键**（§1.3 Phase 3 折中，⬜）— 与 Coalescing 协同  
3. **`streaming_body_forward`**（P3）  
...
已引入 `SsePipeline`（[`sse_pipeline/`](crates/crab-proxy/src/sse_pipeline/)）处理流式 chunk；请求/响应生命周期已拆至 [`phases/`](crates/crab-proxy/src/phases/)，[proxy.rs](crates/crab-proxy/src/proxy.rs) 约 **566 行**（helpers + 瘦 `ProxyHttp` 委托）。原草案：
...
| GenericRelay early exact cache | 延迟 | 低 | 低 | P2 | ✅ |
| MiMo 近似键（sfp + msg count） | 高峰减上游、Coalesce | 中 | 中 | P2 | ⬜ |
| L2 语义缓存（配置启用） | 相似问句命中 | 中 | 中 | 运维+P2 | 🟡 |
| 自适应后端权重 | 可用性 | 高 | 高 | P2 | ⬜ |
...
**非本矩阵**（见 [OPS_RUNBOOK.md](docs/OPS_RUNBOOK.md)）：上游 Key 池、per-key in-flight、`limit_source`、Trace 截断、Redis 启动竞态、Cursor 并行治理。

---

## 附录：proxy.rs 拆分建议 — ✅ 已做（P2+）

当前 [proxy.rs](crates/crab-proxy/src/proxy.rs) 约 **566 行**（`GatewayProxy` helpers + 瘦 `ProxyHttp` 委托）；请求/响应各阶段在 [phases/](crates/crab-proxy/src/phases/)。流式 chunk 处理仍在 `sse_pipeline/`。原目标结构：
